...
import { 
  GitBranch, History, ExternalLink, Activity, 
  Search, MoreHorizontal, CheckCircle2, Loader2, XCircle
} from "lucide-react";
...
export default function ProjectOverview() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch project details and deployment history concurrently
  ...
  const handleRollback = async (targetDeploymentId) => {
    ...
    try {
      ...
      // Refresh the page data to show the new active deployment
  ...
  // Separate the active production deployment from the rest
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-foreground font-sans">
      ...
      <nav className="sticky top-0 z-30 h-14 border-b border-foreground/10 bg-background/80 backdrop-blur-md px-6 flex items-center justify-between">
        ...
        <button className="p-1.5 hover:bg-foreground/10 rounded-md transition-colors">
      ...
      <div className="max-w-6xl mx-auto p-6 lg:p-10 space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <section className="space-y-4">
          ...
          <div className="bg-[#000000] border border-foreground/10 rounded-xl overflow-hidden shadow-2xl flex flex-col lg:flex-row min-h-[350px]">
            ...
            <div className="w-full lg:w-1/2 border-b lg:border-b-0 lg:border-r border-foreground/10 relative overflow-hidden bg-white/5 flex items-center justify-center group">
              {activeDeployment?.status === "READY" ? (
                <div className="absolute inset-0 overflow-hidden bg-white">
                  ...
                  />
                  {/* Subtle overlay on hover to indicate it's a preview */}
        ...
        {/* 3. Active Branches / Preview Deployments */}
